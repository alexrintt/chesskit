...
SquareSet bishopAttacks(Square square, SquareSet occupied) {
  final SquareSet bit = SquareSet.fromSquare(square);
...
SquareSet ray(Square a, Square b) {
  final SquareSet other = SquareSet.fromSquare(b);
...
final List<SquareSet> _kingAttacks = _tabulate(
  (Square sq) => _computeRange(sq, <int>[-9, -8, -7, -1, 1, 7, 8, 9]),
);
final List<SquareSet> _knightAttacks = _tabulate(
  (Square sq) => _computeRange(sq, <int>[-17, -15, -10, -6, 6, 10, 15, 17]),
);
final Map<Side, List<SquareSet>> _pawnAttacks = <Side, List<SquareSet>>{
  Side.white: _tabulate((Square sq) => _computeRange(sq, <int>[7, 9])),
  Side.black: _tabulate((Square sq) => _computeRange(sq, <int>[-7, -9])),
};

final List<SquareSet> _fileRange = _tabulate(
  (Square sq) => SquareSet.fromFile(squareFile(sq)).withoutSquare(sq),
);
final List<SquareSet> _rankRange = _tabulate(
  (Square sq) => SquareSet.fromRank(squareRank(sq)).withoutSquare(sq),
);
final List<SquareSet> _diagRange = _tabulate((Square sq) {
  final int shift = 8 * (squareRank(sq) - squareFile(sq));
  return (shift >= 0
          ? SquareSet.diagonal.shl(shift)
          : SquareSet.diagonal.shr(-shift))
      .withoutSquare(sq);
});
final List<SquareSet> _antiDiagRange = _tabulate((Square sq) {
  final int shift = 8 * (squareRank(sq) + squareFile(sq) - 7);
  return (shift >= 0
...
SquareSet _rankAttacks(Square square, SquareSet occupied) {
  final SquareSet range = _rankRange[square];
  final SquareSet bit = SquareSet.fromSquare(square);
  SquareSet forward = occupied & range;
