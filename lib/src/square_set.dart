...
class SquareSet {
  ...
  static const SquareSet empty = SquareSet(0);
  static const SquareSet full = SquareSet(0xffffffffffffffff);
  static const SquareSet lightSquares = SquareSet(0x55AA55AA55AA55AA);
  static const SquareSet darkSquares = SquareSet(0xAA55AA55AA55AA55);
  static const SquareSet diagonal = SquareSet(0x8040201008040201);
  static const SquareSet antidiagonal = SquareSet(0x0102040810204080);
  static const SquareSet corners = SquareSet(0x8100000000000081);
  static const SquareSet center = SquareSet(0x0000001818000000);
  static const SquareSet backranks = SquareSet(0xff000000000000ff);
  ...
  SquareSet flipVertical() {
    const int k1 = 0x00FF00FF00FF00FF;
    const int k2 = 0x0000FFFF0000FFFF;
  ...
  SquareSet mirrorHorizontal() {
    const int k1 = 0x5555555555555555;
    const int k2 = 0x3333333333333333;
    const int k4 = 0x0f0f0f0f0f0f0f0f;
  ...
  String toString() {
    ...
    final String first = int.parse(b.substring(0, 32), radix: 2)
        .toRadixString(16)
        .toUpperCase()
        .padLeft(8, '0');
    final String last = int.parse(b.substring(32, 64), radix: 2)
        .toRadixString(16)
        .toUpperCase()
        .padLeft(8, '0');
    final String stringVal = '$first$last';
  ...
  Iterable<Square> _iterateSquares() sync* {
    int bitboard = value;
    while (bitboard != 0) {
      final int? square = _getFirstSquare(bitboard);
      bitboard ^= 1 << square!;
      yield square;
    }
  }
  ...
  Iterable<Square> _iterateSquaresReversed() sync* {
    int bitboard = value;
    while (bitboard != 0) {
      final int? square = _getLastSquare(bitboard);
      bitboard ^= 1 << square!;
      yield square;
    }
  }
...
int _popcnt64(int n) {
  final int count2 = n - ((n >>> 1) & 0x5555555555555555);
  final int count4 =
      (count2 & 0x3333333333333333) + ((count2 >>> 2) & 0x3333333333333333);
  final int count8 = (count4 + (count4 >>> 4)) & 0x0f0f0f0f0f0f0f0f;
